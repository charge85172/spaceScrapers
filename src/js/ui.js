import { Label, Color, Vector, Actor, Rectangle } from "excalibur"
...
export class ScoreUI {
    ...
    removeAllUI(engine) {
        // Remove all UI actors from the engine
        ...
        // Remove restart handler if present
    ...
    showWinner(engine, winner) {
        if (!this.gameOverLabel) {
            ...
            this._restartHandler = (evt) => {
                if (evt.key === ' ') {
                    this.hideGameOver(engine)
                    engine.scoreUI.reset()
                    engine.startGame()
                }
            }
